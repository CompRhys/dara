...
function Submission() {
  ...
  const [previousInstrumentSelected, setPreviousInstrumentSelected] = useState(
    "Aeris-fds-Pixcel1d-Medipix3",
  );
  ...
  const onSubmit = () => {
    ...
    for (const key in values) {
      ...
      if (key === "precursor_formulas") {
        formData.append(
          key,
          JSON.stringify(values[key].split(",").map((item) => item.trim())),
        );
      ...
      <Container>
        ...
        >
          <Form.Item
            label="User"
            name="user"
            tooltip="The username for this refinement. It can be any string that will be shown with the result later."
          ...
          <Form.Item
            ...
            tooltip="Whether to use reaction-network to filter the phases based on energy."
          >
          ...
          <Form.Item
            label={rxn_ntwk ? "Precursors Formulas" : "Included elements"}
            name="precursor_formulas"
            rules={[{ required: true, message: "Please input the formula!" }]}
            tooltip={rxn_ntwk ? "Example: Bi2O3, Fe2O3" : "Example: Bi,Fe,O"}
          >
          ...
          <Form.Item
            ...
            name="temperature"
            rules={[
              { required: rxn_ntwk, message: "Please input the temperature!" },
            ]}
            hidden={rxn_ntwk ? false : true}
          >
          ...
          >
            <Select
              ...
              options={INSTRUMENT_NAMES.map((name) => ({
                label: name,
                value: name,
              }))}
            />
          ...
          >
            <Switch
              onChange={(checked) => {
                ...
                if (checked) {
                  setPreviousInstrumentSelected(
                    form.getFieldValue("instrument_profile"),
                  );
                  form.setFieldsValue({ instrument_profile: "synchrotron" });
                } else {
                  form.setFieldsValue({
                    instrument_profile: previousInstrumentSelected,
                  });
                }
              }}
            />
          ...
          {!synchrotron ? (
            <Form.Item
              label="Wavelength"
              name="wavelength_lab"
              tooltip="The wavelength in nm of X-ray for collecting the pattern."
              rules={[
                { required: true, message: "Please input the wavelength!" },
              ]}
            >
              <Select
                showSearch
                ...
                options={WAVELENGTHS.map((name) => ({
                  value: name[0],
                  label: `${name[0]} (${name[1].toFixed(4)} Å)`,
                }))}
              />
            </Form.Item>
          ) : (
            <Form.Item
              ...
              tooltip="The wavelength of X-ray for collecting the pattern."
              rules={[
                {
                  required: true,
            ...
            >
              <Input
                placeholder="The wavelength of X-ray for collecting the pattern."
                suffix="Å"
              />
            </Form.Item>
          )}
