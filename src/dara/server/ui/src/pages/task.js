...
function RefinementPlot({ task_id, plot_id }) {
  ...
  const request_url =
    plot_id === null
      ? `${URL}/task/${task_id}/plot`
      : `${URL}/task/${task_id}/plot?idx=${plot_id}`;
  React.useEffect(() => {
  ...
  return plot === undefined ? (
    ...
    <Plotly {...JSON.parse(plot)} style={{ height: "100%", weight: "100%" }} />
  );
}
      ...
      renderItem={(item, index) => (
        <List.Item key={index}>
          <List.Item.Meta
            ...
            description={
              composition_grouping ? (
                grouped_phases[index].map((groupedItem, idx) => (
                  <div key={idx} className="phases-container">
                    <span className="representative-phase">
                      {parse(groupedItem[0])}
                    </span>{" "}
                    - (
  ...
  return (
    <>
      <TitleContriner>
        <h3>
          Task details (
          <a onClick={() => setPlotId(null)}>Visualize final result</a>)
        </h3>
        {/* <Button type="primary" href={`${URL}/task/${task_id}/download`} target="_blank">Download Project File</Button> */}
      </TitleContriner>
      <Divider />
      <Descriptions bordered column={2} size="small">
        <Descriptions.Item label="Pattern file name" span={2}>
          {data.task_label}
        </Descriptions.Item>
        <Descriptions.Item label="Best_Rwp" span={2}>
          <Tag color="green">{data.best_rwp} %</Tag>
        </Descriptions.Item>
        <Descriptions.Item label="Temperature" span={2}>
          {data.temperature || "None"}
        </Descriptions.Item>
        <Descriptions.Item label="Use reaction network?" span={2}>
          {data.use_rxn_predictor ? "Yes" : "No"}
        </Descriptions.Item>
        <Descriptions.Item label="Additional search options" span={2}>
          <code style={{ backgroundColor: "transparent" }}>
            {JSON.stringify(data.additional_search_options)}
          </code>
        </Descriptions.Item>
        <Descriptions.Item label="Compositions" span={2}>
          {data.precursors}
        </Descriptions.Item>
        <Descriptions.Item label="Final phases" span={2}>
          {data.final_result.phases.map((item, index) => (
            <span>
              {index ? " | " : ""}
              <code>{item}</code>
            </span>
        ...
        <Descriptions.Item label="Duration" span={2}>
          <Tag color="cyan">{data.runtime.toFixed(2)} s</Tag>
        </Descriptions.Item>
      </Descriptions>
      <ResultTitleContainer>
        ...
        <div>
          ...
          <Switch
            defaultChecked
            onChange={setCompositionGrouping}
          ...
          return (
            <>
              <Descriptions.Item
                ...
                label={
                  <>
                    {`Search result ${index + 1} (Rwp = ${result.rwp} %)`}
                    <br />(
                    <a onClick={() => setPlotId(index)}>Visualize result</a>)
                  </>
                }
                span={2}
              >
                ...
                <PhaseList
                  ...
                  highlighted_phases={result.highlighted_phases}
                  grouped_phases={result.grouped_phases}
      ...
      <Modal
        title={`Refinement result ${plot_id === null ? "(final)" : plot_id + 1}`}
        open={plot_id !== undefined}
        onCancel={() => setPlotId(undefined)}
        footer={[]}
        width={"70vw"}
      >
  ...
  return (
    <Descriptions bordered column={1} size="small">
      ...
      <Descriptions.Item label="Status" span={1}>
        <Tag color={STATE_COLOR[status]}>{status}</Tag>
      </Descriptions.Item>
      <Descriptions.Item label="Submitted on" span={1}>
        {submitted_on}
      </Descriptions.Item>
      {start_time ? (
        <Descriptions.Item label="Start time" span={1}>
      ...
      {error_tb ? (
        <Descriptions.Item label="Error Traceback" span={1}>
          <div class="error-code">
            <pre style={{ whiteSpace: "break-spaces", fontSize: "0.9em" }}>
              {error_tb}
            </pre>
          </div>
        </Descriptions.Item>
    ...
    <Layout hasSider={false} title="Result">
      <Container>
        ...
        {data && data.status === "COMPLETED" ? (
          <TaskDetail data={data} task_id={task_id} />
        ) : (
          <></>
        )}
        {data && data.status !== "COMPLETED" ? (
          <TaskDetailedUnfinished data={data} />
        ) : (
          <></>
        )}
      </Container>
